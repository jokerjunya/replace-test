...
// --- Mock Data (Copied and stripped of UI components) ---
...
async function main() {
    ...
    const collectTraits = (traits: any[]) => {
        traits.forEach(t => allTraits.set(t.id, t));
    };

    MOCK_TEAMS.forEach(team => {
        collectTraits(team.leader.traits);
        team.members.forEach(m => collectTraits(m.traits));
    });
    MOCK_CANDIDATES.forEach(c => collectTraits(c.traits));

    for (const trait of allTraits.values()) {
    ...
    // 2. Seed Teams & Employees
    for (const teamData of MOCK_TEAMS) {
        const team = await prisma.team.upsert({
            ...
            create: {
                ...
                themeColor: teamData.themeColor,
                psychologicalSafety: teamData.metrics.psychologicalSafety,
                productivity: teamData.metrics.productivity,
                innovation: teamData.metrics.innovation,
            },
        })
        ...
        const leaderUser = await prisma.user.upsert({
            where: { email: `${leaderData.id}@example.com` },
            update: {},
            create: {
                email: `${leaderData.id}@example.com`,
        ...
        await prisma.employee.upsert({
            where: { id: leaderData.id },
            update: {
                leadingTeam: { connect: { id: team.id } },
                team: { connect: { id: team.id } },
            },
            create: {
                id: leaderData.id,
                user: { connect: { id: leaderUser.id } }, // Fixed
                firstName: leaderData.name.split(' ')[0] || '',
                lastName: leaderData.name.split(' ')[1] || '',
                role: leaderData.role,
                avatarUrl: leaderData.avatar,
                stressLevel: leaderData.stressLevel,
                performance: leaderData.performance,
                team: { connect: { id: team.id } },
                leadingTeam: { connect: { id: team.id } },
                traits: {
                    create: leaderData.traits.map(t => ({
        ...
        for (const memberData of teamData.members) {
            const memberUser = await prisma.user.upsert({
                where: { email: `${memberData.id}@example.com` },
                update: {},
                create: {
                    email: `${memberData.id}@example.com`,
                    passwordHash: 'hashed_password',
                    role: 'EMPLOYEE',
                },
            });
            ...
            await prisma.employee.upsert({
                ...
                create: {
                    id: memberData.id,
                    user: { connect: { id: memberUser.id } }, // Fixed
                    ...
                    team: { connect: { id: team.id } },
                    traits: {
                        create: memberData.traits.map(t => ({
                            trait: { connect: { id: t.id } },
                            weight: 1.0
                        }))
                    }
    ...
    console.log('Teams and Employees seeded.')

    // 3. Seed Candidates
    for (const candidateData of MOCK_CANDIDATES) {
        const candidateUser = await prisma.user.upsert({
            where: { email: `${candidateData.id}@example.com` },
            update: {},
            create: {
                email: `${candidateData.id}@example.com`,
                passwordHash: 'hashed_password',
                role: 'USER',
            },
        });
        ...
        const candidate = await prisma.candidate.upsert({
            ...
            create: {
                id: candidateData.id,
                user: { connect: { id: candidateUser.id } }, // Fixed
                firstName: candidateData.name.split(' ')[0] || '',
                lastName: candidateData.name.split(' ')[1] || '',
                role: candidateData.role,
                avatarUrl: candidateData.avatar,
                status: candidateData.status.toUpperCase(),
                resumeJson: JSON.stringify(candidateData.details?.resume),
                preferencesJson: JSON.stringify(candidateData.details?.preferences),
                traits: {
                    create: candidateData.traits.map(t => ({
                        trait: { connect: { id: t.id } },
                        weight: 1.0
                    }))
                }
        ...
        for (const [teamId, score] of Object.entries(candidateData.matchScores)) {
